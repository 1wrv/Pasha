...
g= datetime.datetime(2023, 11, 29, 0, 0 )
...
def setting():
	...
	print('\x1b[38;5;208m  ⚜️ النسخة تحت الفحص ليست كاملة  ⚜️                      ')
...
def crack(idf,pwv):	
	...
	fff = '%'
	prog.update(des,description=f'\r [blue]⚜️ 𝑹𝑴𝑨𝑫 ⚜️ [yellow] [deep_white]{(loop)}/{len   (id)}[blue]🔹𝐎𝐊  🧸 [/][green] {(ok)} [/][purple][purple] 🧸 𝐶𝑃 🔹 [/] :[purple]{(cp)}')
	prog.advance(des)
	ses = requests.Session()
...
def getkey():
    ...
    try:
        ...
        print("")
        print('')
        print("\033[0m Telegram: @R_M_D ");time.sleep(2)
        print ("")
...
def file_cp():
	dirs = os.listdir('CP')
	print ("%s%s%s [%s\033[0m \033[0m⚜️ 𝑹𝑴𝑨𝑫 ⚜️ hasil crack yg tersimpan untuk cek opsi %s]\n"%(U,til,O,U,O))
	for file in dirs:
